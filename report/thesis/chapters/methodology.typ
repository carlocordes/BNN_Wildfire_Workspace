...
@fig:temporal_scope illustrates the terms defined above. It shows two consecutive sequences as well as their respective samples and targets. It shows the temporal division of the sample into subsequent timesteps. The distance between sample and target corresponds to the above definition of #emph[lead-time].
...
=== Temporal Embedding
Weighing patches of the same modality against each other should be entirely up to the transformer model. Afterall, the idea is to find nuanced temporal relationships both within a modularity and between pairs of them. To enforce distinct temporal relationships between dynamic tokens as well as a global perception of the model for seasonality we include an additional temporal encoding for all dynamic modules. Used here is an encoding of the Julian day, which is simply an integer number of the day of the year (0-365). To make use of this in the best way, we encode the Julian day so that it captures the true nature of the climatological conditions: its cyclical nature. The embedding of a patch from a day in December should be close to one in the beginning of January, despite their Julian day being far apart.
