...
#figure(
  table(
    ...
    [MODIS/061/MOD09GQ], [Terra MODIS Level-2G Surface Reflectance  (NDVI)], [Daily], [2000-01-01], [250 m], [Global ($90 degree upright(N)$ to $90 degree upright(S)$)],

    [MODIS/061/MOD09GA], [Terra MODIS Level-2G Surface Reflectance (NDWI)], [Daily], [2000-01-01], [500 m], [Global ($90 degree upright(N)$ to $90 degree upright(S)$)],

    [UCSB-CHG/CHIRPS/DAILY], [CHIRPS rainfall estimates.], [Daily], [1981-01-01], [\~5,500 m], [Quasi-global ($50 degree upright(N)$ to $50 degree upright(S)$)],
  ),
...
=== Road Proximity
As discussed in @chap:related_work, the main driver of wildfire ignition is human activity. This information is implicitly encoded via proximity to roads. The basis of this serves a vector dataset that encoded roads for the entirety of Europe, which was synthesized as part of a study to describe patterns in road infrastructure @roads_dataset. 

The process here includes selecting roads in the area of interest and creating a small buffer around them, essentially converting from #emph[LineString] to #emph[Polygon]. These are then converted in a binary road map raster, which has exact dimensions as all other inputs, as dictated by the #emph[Golden Grid]. Ones indicate roads, zeros indicate any other land use. On the basis of this we conduct a grid search for every pixel in the image. Here we compute the physical proximity from the center of each pixel to the nearest road pixel. 


=== Ratio-Scale data types
// Split Interval scales into ratio scales (which have absolute zero)
Processing the digital terrain model (DTM) as a raw feature is possible, and is valid, however we enrich terrain information passed to the model by additionally including derivatives of it. After all it is less the total altitude a model is interested in when predicting fire, but more the contextual appearance. Therefore, the terrain is processed into slope and aspect maps. 
...
We additionally must treat the datasets missing data points. Many of the datasets are influenced by cloud-cover. Those relying on near-infrared data like NDVI, which does not penetrate cloud cover long stretches of missing data can be observed especially in the winter months. Regardless of its relevance to a wildfire prediction, we additionally mask out waterbodies on all images. Not all modularities produce values over oceans and rivers, making it a safer option to remove them entirely. A joint no-data value both for missing data and water-cover of $-1$ has been chosen as a unified value. Through its learning process, the transformer will learn to give especially those pixels over water less weight, effectively removing them from the data scope.
