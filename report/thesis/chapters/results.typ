...
As a proof of concept and a discussion of *RQ 1* in @fig:research_questions, here follows a description of the choices made to yield a working model that can confidently predict wildfire occurrence probability.
...
A more detailed view into the results of the prediction pipeline is showcased in @fig:unified_comaparison. Here we illustrates Portugal, the region of interest in this work from 3 perspectives. The left shows an orthophoto taken from Sentinel-2, the middle shows wildfire risk predictions made for the same time window and the right side illustrates the ground truth or the target, with which the predictions were trained against for this exact timestep. Note that the few burn pixels in the ground truth for this step alone do not account for all risk regions identified by the model, as it has by this point learned to infer critical wildfire conditions from other regions. Additionally, this figure for each shows further zoomed in regions in the rows below. Zoom region A in the second row of the image illustrates the ideal outcome case of such a predictive transformer. Not only has it for this timestep correctly identified a high-risk region, the burn probability of $3 percent$ in this location even coincides with an actual observed burn event. As visible from the top row, predicting fires correctly is rather a rare circumstance. Since we predict risk and not direct fire events, this is expected. Zoom region B is yet another good example for proficient identification of land and water mass. While not observing a direct fire event for this region, the predictions indicate a significant level of risk, which happens to perfectly depict the coastal region, using the waterbodies as a natural barrier.

#figure(
  image("../figs/unified_comparison.png"),
  caption : [Complete and zoomed view of Sentinel-2 orthophoto, predictions and ground truth (09. August 2025)]
) <fig:unified_comaparison>


=== Return Period Analysis
Given the abstraction of a deterministic ground truth into a probabilistic prediction, it becomes hard to obtain direct comparisons. Afterall, regardless of a fire happening at a certain position in space, model training does not largely penalize a prediction of risk, where no fire was seen. This being the basis of the infered wildfire risk, we lose the ability to draw sensible comparisons between prediction and ground truth, making especially a discussion of concrete error unfeasible. 

To subsidize the discussion of model validity we here present a return period approach. Conceptually, it can be argued, that while in the daily prediction perspective, ground truths will not be perfect matches, this effect should ideally vanish when accumulating predictions over large periods of time. We here test the baseline model against 10 years of data and accumulate its predictions. By summing per-pixel probabilities, we first obtain an estimate of the expected number of fires over this period and from this derive the return period, or the expected amount of years between burns per pixel. Cells with a higher return period burn more rarely than those with a low return period. 
#figure(
  image("../figs/return_pd_compare.png", width : 100%),
  caption : [Comparison of burn return period per pixel (truth vs. prediction)]
) <fig:return_period>

@fig:return_period shows the accumulated return period of the baseline model along with the same data as collected from ground truth data of 10 years. The caveat here is that the ground truth only contains 26 years of burn data making a comparison somewhat arbitrary. However, the model predictions are more dynamic. By summing regions with low aggregate risk we are even able to establish return periods upwards of 100 years, without ever having observed fires from the much shorter ground truth data. Analyzing entirely from a visual perspective, comparison of the two images shows that in the aggregate the model correctly captures the hotspots of fire activity. Especially in the north-western region hotspots appear similarly in both will also showing the high-risk region of the south-western peninsula of Sagres. While mostly without historical records, the central region is sparsely covered by risk, as correctly indicated by the ground truth. 
...
To further demonstrate the models abilities and illustrate their differences, @fig:samp_lift illustrates cumulative gains of lift-charts of the models. Essentially, the methodology here argues that for such a rare event like wildfires, most of the risk should be contained in a fraction of the total area. The set of predictions, presented on the x-axis, has here been ordered in descending order of predicted risk. A model that would simply guess randomly would show a uniform distribution of risk over all pixels, as denoted by the random spatial baseline. A well-trained model on the other hand should be able to contain large amounts of the actual fires in very few image pixels.

All four models show this behavior, proving they operate without having to guess randomly. The clear categorical winner from this perspective is again #emph[Sample_Extent3], with the highest overall Gini-coefficient of $0.620$. It is able to contain roughly $70%$ of actual fires within $20%$ of the total area. 
...
Most interesting in this case is the testing loss chart, which shows a staircase behavior, when elongating the lead-time. This shows a very clear patter: the longer the lead-time, the more predictive accuracy degrades for unseen data. Despite its validation loss being exceptional ($0.3193$), #emph[Lead_time20] far overfits the data as can be identified by its test loss value ($0.8852$). 
...
The global sensitivity analysis by data showing the ablation results for all 12 configurations is shown in @fig:ablation_seasonal. The most important confirmation here is one that as was expected from discussions in @chap:related_work, terrain is a main driver of prediction robustness. Removing the combined aspect and slope modules (MAE: $0.07080$, MSE: $0.01336$) removes all systematic overview of the model, making it the single most important metric of the model. Another notable result is that it by removing just one of the two terrain types (aspect or slope), the error is somewhat contained. Both modularities appear to be appear an error safeguard for the respective other, as described by their comparably small MSE (Aspect: $0.00102$, Slope: $0.00029$). The dominating feature under the dynamic features is LST (MAE: $0.05223$, MSE: $0.00945$). It acts as the anchor of thermal context, naturally scaling wildfire probabilities.

Roads (MSE: $0.03627$), burn history (MSE: $0.03258$) and wind components (MSE: $0.02941$) represent the second tier of feature importance. They don't carry copious amounts weight, but in combination provide the model with critical boundary logic. While both burn history and road proximity are not dynamic features, but aid the prediction crucial secondary information regarding proximity to civilization.
...
The key notion shown in @fig:ablation_seasonal is that the induced error from removing modalities is variable by season. For most ablations, the model prediction error remains stable for the first nine indices, or about four months. This is the probable time for wildfire occurrence, hence none of the conditions in any module are fulfilled to cause significant risk. Quite clearly, since the model is predicting imbalanced binary classes, it requires less information to predict the more likely class as more information exists for it. For all ablations considered, MAE shifts rapidly as soon as wildfires are in season, at around indices 9-10. It is at this point, that the structural reasoning changes from relying on the broadness of all modules in winter, to predicting higher probabilities from a few extreme values in the summer months. Perhaps most notably, removing terrain features and LST causes significant errors of up to $16 percent$ in peak season. Furthermore, LST and terrain features show correlating error behavior in their peaks from indices 14-16, stating that during this peak season, the features interact with one another.

Features contributing significantly less error especially in the seasonal perspective are NDVI and wind speed despite both of these being large datasets producing significant amounts of input tokens. Both stay almost perfectly level, causing little inaccuracies to the model when omitted. Rather than being heavily reliant on them, the model treats these features as a baseline offset and does not adjust them by season.

An important take-away from this ablation study, especially when evaluating RQ4, is that evaluating feature importance is entirely possible. By showing a distributed feature reliance, it is clear that the model does not simply learn via shortcuts, depending on a single variable to produce as little loss as possible, but uses the entire arsenal of its data types. @fig:ablation_absolute showed that features do have relative levels of impact on the prediction outcome, while none exceed an error of $7 %$. This shows that the cross-attention backbone of the architecture is successfully using information from all modalities. Another important argument to be made, is that despite dealing with immense cloud-cover in the winter months, all dynamic weather-dependent data types suppress this noise, producing no larger error than the static features. The model is however to some extent sensitive to imbalance of classes, which can be detected from rising error for all module ablations in fire-prone months. 

Speaking for an idea broader than just the results, the integrated architecture and evaluated in this section inherits trains that go beyond simply applying regressions to large amounts of input data. Not only has the model learnt how to interpret and apply data and their subsequent embeddings, it has formed a much richer, interpreted idea of the underlying phenomenon. This effect can be directly related back to the model architecture in @fig:architecture. The final predictions stem not from single data modules as well, but from the fused attention backbone which relies on information learnt not many repetitions. As for humans, reasoning becomes much more difficult with omitted information, which is the effect the transformer is demonstrating here. It would not be applicable to claim that the transformer model is behaving intelligently in its predictions, however during training it has successfully been able to internalize the conditions of fire-prone regions and makes use of its highly dimensional embedding space to store as many diverse conditions as possible. 