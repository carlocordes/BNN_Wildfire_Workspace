...
class SpatialTemporalDataset(Dataset):
    def __init__(self, config_path: Path, split_type: str = "train", benchmark_mode = False):
        ...
        if split_type == "train":
            self.sequences = [s for s in all_sequences if s['year'] in [2014]]
        elif split_type == "val":
            self.sequences = [s for s in all_sequences if s['year'] in [2000, 2001, 2002, 2003, 2004,
            ...
            self.sequences = [s for s in all_sequences if s['year'] == 2013]
        else:
